 ...
 * @version 1.0
 */
public class NestTableCtl extends AbstractMacroCtl implements INestTableCtl {
    ...
    public String getNestTable(HttpServletRequest request, FormField ff) {
        ...
        if ("archive".equals(pageType)) {
            // 加上skey，否则会被重定向到登录页面
            IMobileService mobileService = SpringUtil.getBean(IMobileService.class);
            String skey = mobileService.generateSkey(SpringUtil.getUserName());
            url += "&skey=" + skey;

            SysProperties sysProperties = SpringUtil.getBean(SysProperties.class);
            String path = sysProperties.getRootPath() + url;
        ...
        str += "<script>\n";
        str += "async function ctlFunc_" + ff.getName() + "() {\n";

        if (request.getAttribute("isNestTableCtlJS") == null) {
            str += "await ajaxGetJS(\"/flow/macro/macro_js_nesttable.jsp?isTab=" + isTab + "&pageType=" + pageType + "&fieldName=" + ff.getName() + "&nestFormCode=" + nestFormCode + "&flowId=" + cwsId + "\", {});\n";
        ...
        if (!"show".equals(pageType) && !"flowShow".equals(pageType)) {
            ...
            // if (request.getAttribute("isNestTableCtlJS_forRefresh") == null) {
                str += "await ajaxGetJS(\"/flow/macro/macro_js_nesttable.jsp?op=forRefresh&pageType=" + pageType + "&isTab=" + isTab + "&fieldName=" + ff.getName() + "&parentFormCode=" + parentFormCode + "&nestFormCode=" + nestFormCode + "&path=" + StrUtil.UrlEncode(ajaxPath) + "&flowId=" + cwsId + "&mainId=" + mainId + "\", {});\n";
        ...
        // } else {
        str += "await loadNestCtl('" + url + "','nestsheet_" + ff.getName() + "');\n";

        str += "}\n";
        str += "ctlFunc_" + ff.getName() + "();\n";
        str += "</script>";
        return str;
    }
    ...
    public String getFormCode(HttpServletRequest request, FormField formField) {
        String nestFormCode = formField.getDescription();
        try {
            String defaultVal = StrUtil.decodeJSON(formField.getDescription());
            JSONObject json = new JSONObject(defaultVal);
            nestFormCode = json.getString("destForm");
        } catch (JSONException e) {
            LogUtil.getLog(getClass()).error(nestFormCode + " is old version before 20131123. ff.getDefaultValueRaw()=" + formField.getDefaultValueRaw());
            LogUtil.getLog(getClass()).error(e);
        }
        return nestFormCode;
    }
    ...
    public int createForNestCtl(HttpServletRequest request, FormField macroField, String cwsId,
        ...
        // 解析出行中的字段，按td从左至右顺序
        Vector<FormField> fields = parseFieldsFromView(fd, viewContent);
    ...
    public int saveForNestCtl(HttpServletRequest request, FormField macroField, String cwsId,
        ...
        if (rowIds==null) {
            ...
            try {
                return jt.executeUpdate(sql, new Object[]{String.valueOf(fdaoId), macroField.getFormCode()});
            } catch (SQLException e) {
                LogUtil.getLog(getClass()).error(e);
            }
        }
        ...
        if (request.getAttribute("isSaveDraft")!=null) {
            isSaveDraft = (Boolean)request.getAttribute("isSaveDraft");
        }
        if (!isSaveDraft) {
            ParamChecker pck = new ParamChecker(request, fu);
            for (int i = 0; i < rows; i++) {
        ...
        String sql = "select id from " + fd.getTableNameByForm() +
                " where cws_id='" + fdaoId + "' and cws_parent_form=" + StrUtil.sqlstr(macroField.getFormCode()) +
                " order by cws_order";
        ...
        DeptUserDb dud = new DeptUserDb();
        String unitCode = dud.getUnitOfUser(creator).getCode();
        FormDAO fdaoForValidtae = new FormDAO(fd);
                ...
                while (irDao.hasNext()) {
                    ...
                    if (fdao.getId() == dataId) {
                        ...
                        for (Object o : fieldsWritable) {
                            ...
                            // 当后面验证校验规则时，需赋值给fileUpload中对应的属性
                            fu.setFieldValue(ff.getName(), ff.getValue());
        ...
        try {
            if (!"".equals(upFile)) {
                if ("detaillist".equals(nestType)) {
                    ...
                    FormDb fd = new FormDb();
                    fd = fd.getFormDb(nestFormCode);
                    Vector fields = parseFieldsFromView(fd, viewContent);
    ...
    @Override
    public String convertTextarea2InputAndClearCwsSpan(String html) {
        return convertTextareaToInputAndClearCwsSpan(html);
    }
    ...
    public static String convertTextareaToInputAndClearCwsSpan(String html) {
        ...
        try {
            ...
            */
            ...
            LogUtil.getLog(NestTableCtl.class).error(e);
        }
    ...
    /**
     * 为了便于在c-main模块中调用，上面的static方法无法直接调用
     * @param fd
     * @param viewContent
     * @return
     */
    @Override
    public Vector<FormField> parseFieldsByView(FormDb fd, String viewContent) {
        return parseFieldsFromView(fd, viewContent);
    }
