...
public class WorkflowMineSelectCtl  extends AbstractMacroCtl {
	...
	public String convertToHTMLCtl(HttpServletRequest request, FormField ff) {
		...
		if (!StrUtil.getNullStr(ff.getValue()).equals("")) {
			...
			realName = "<a href='javascript:;' onclick=\"addTab('" + wf.getTitle() + "', '" + request.getContextPath() + "/flowShowPage.do?flowId=" + wf.getId() + "&visitKey=" + visitKey + "')\">" + wf.getTitle() + "</a>";
	...
	public String converToHtml(HttpServletRequest request, FormField ff,
		...
		if (!v.equals("")) {
			...
			// 以flowId作为值加密
			String visitKey = cn.js.fan.security.ThreeDesUtil.encrypt2hex(desKey, String.valueOf(wf.getId()));
			String str = "<a href='javascript:;' onclick=\"addTab('" + wf.getTitle() + "', '" + Global.getRootPath() + "/flowShowPage.do?flowId=" + wf.getId() + "&visitKey=" + visitKey + "')\">" + wf.getTitle() + "</a>";
