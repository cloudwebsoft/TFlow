...
public class WorkflowSequenceCtl extends AbstractMacroCtl {
			...
			if (wsd.getType() == WorkflowSequenceDb.TYPE_NUMBER) {
				...
				if (p > -1 && !"".equals(userName)) {
					...
					if (ir.hasNext()) {
						...
						deptShortName = StrUtil.getNullStr(dd.getShortName()); // 发起人所在部门的简称
				...
				value = makeSequence(wsd.getTemplate(), deptShortName, wsd.getLength(), jsonAry, isCurrent, wsd);
	...
	public synchronized String convertToHTMLCtl(HttpServletRequest request,
		...
		return "<input id='" + ff.getName() + "' name='" + ff.getName()
				+ "' value='" + value + "' size='15' readonly='readonly' " + style + " />";
	}
	...
	public Object getValueForSave(FormField ff, int flowId, FormDb fd, FileUpload fu) {
		...
		if (!"".equals(val) && !WORKFLOW_SEQUENCE_NONE.equalsIgnoreCase(val)) {
			return val;
		}
									...
									if (i > -1 && i <= formVal.length()) {
										...
										if (StrUtil.toInt(temp2)
											...
											if (wf.getTitle().contains(formVal)) {
												wf.setTitle(wf.getTitle().replaceAll(formVal, value));
												wf.save();
											}
										} else {
											value = formVal;
	...
	public String getControlValue(String userName, FormField ff) {
		if (ff.isEditable()) {
			if (StrUtil.isEmpty(ff.getValue()) || WORKFLOW_SEQUENCE_NONE.equals(ff.getValue())) {
