<template>
  <div :class="{ 'pl-2 pr-2 pb-2 bg-white': isTab }" class="w-full">
    <Spin :spinning="confirmLoading">
      ...
      <QichachaSearchListDrawer
        @register="registerQichachaSearchListDrawer"
        @success="handleQichachaSearchListSuccess"
      />
...
<script lang="ts">
  ...
  import { submitMyFile, downloadFile, getFlowInit } from '/@/api/process/process';
  ...
  import QichachaSearchListDrawer from './qichacha/qichachaSearchListDrawer.vue';
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      async function getFirstTabInfo() {
        ...
        console.log('getFirstTabInfo params', params);
      ...
      function initWindowFunc() {
        ...
        newWindow.handleQichacha = handleQichacha;
      }
      ...
      //企查查查询列表抽屉
      const [registerQichachaSearchListDrawer, { openDrawer: openQichachaSearchListDrawer }] =
        useDrawer();
      //企查查入口
      const handleQichacha = (isSimple = false) => {
        openQichachaSearchListDrawer(true, { isSimple: isSimple });
      };
      //企查查回调
      const handleQichachaSearchListSuccess = (record) => {
        console.log('qichacha-record', record);
        if (typeof window.getCompany === 'function') {
          window.getCompany(record);
        }
      };
      return {
        getTitle,
        ...
        handleQichacha,
        registerQichachaSearchListDrawer,
        handleQichachaSearchListSuccess,
      };
    },
  });
</script>
