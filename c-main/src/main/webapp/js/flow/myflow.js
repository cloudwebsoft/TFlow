(function ($) {
    ...
    myflow.rect = function (o, r, id) {
        ...
        function resize() {
            ...
            console.log('_o.showType', _o.showType);
            switch (_o.showType) {
                case 'image':
                    _img.attr({
    ...
    myflow.init = function (c, o) {
        defaultMaxRectCount = Default_RECT_COUNT; // 默认节点数为5
        maxRectCount = 50000;
        ...
        var addpathHandler = function (e, from, to, dots, isReturn, lineType) {
            ...
            if (inDegree > 0 && to.getType() == ACTION_TYPE.START) {
                // 检查所有的节点，如果还有其它为START的节点，则本节点仍保持为开始节点，否则置为普通任务节点
        ...
        function decryptByDES(ciphertext, key) {
            if (key == '') {
                console.log('decryptByDES: Key is empty.');
                return '';
            }
            // console.log('key=' + key);
            var keyHex = CryptoJS.enc.Utf8.parse(key);
