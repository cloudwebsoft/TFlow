...
public class ModulePrivDb extends ObjectDb {
    ...
    public void initDB() {
        ...
        QUERY_LOAD =
                "select form_code,name,priv_type,priv_see,priv_append,priv_manage,priv_modify,priv_view,priv_search,priv_re_active,field_write,field_hide,priv_import,priv_export,priv_del,priv_log,priv_export_word,priv_data,is_filter,filter_cond,zip,copy,roll_back,priv_export_col,priv_set_list from visual_module_priv where id=?";
        QUERY_DEL = "delete from visual_module_priv where id=?";
        QUERY_SAVE =
                "update visual_module_priv set priv_see=?,priv_append=?,priv_manage=?,priv_modify=?,priv_view=?,priv_search=?,priv_re_active=?,field_write=?,field_hide=?,priv_import=?,priv_export=?,priv_del=?,priv_log=?,priv_export_word=?,priv_data=?,is_filter=?,filter_cond=?,zip=?,copy=?,roll_back=?,priv_export_col=?,priv_set_list=? where id=?";
        QUERY_CREATE =
