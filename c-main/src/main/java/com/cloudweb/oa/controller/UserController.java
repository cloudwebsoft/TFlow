    ...
    public Result<Object> list(String searchType, @RequestParam(defaultValue = "") String op, @RequestParam(defaultValue = "") String condition, @RequestParam(defaultValue = "isValid") String isValid,
        ...
        if (ConstUtil.DEPT_ROOT.equals(deptCode)) {
            ...
            if ("search".equals(op)) {
                ...
                if (isBelongToDept) {
                    sql = "select DISTINCT u.name,u.orders,du.orders as duOrders from users u,department d,dept_user du where u.name = du.user_name and d.code = du.dept_code and isValid = 1 and u.name<>" + StrUtil.sqlstr(ConstUtil.USER_SYSTEM);
                }
                else {
